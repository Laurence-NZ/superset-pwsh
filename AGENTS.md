...
- **Stale `active` terminal sessions accumulate on Windows** because the pty-daemon is spawned `detached` + `unref()`'d and does NOT die with the app — on a plain "Close Superset" it's intentionally left running for reattach, and before the fix below a "Quit Completely" left it orphaned too. Either way a terminal's `onExit` (which flips its `terminal_sessions` row to `exited`) isn't delivered to the exiting host-service, and the reaper only visits sessions the daemon still lists. Host-service startup now sweeps all `active` rows to `exited` on win32 (`packages/host-service/src/app.ts`, before `deleteDefunct()`) — safe because no pty survives an app restart. Without it, each dead row keeps its agent binding live-joined and the workspace-agents sidebar stacks a phantom "Claude" chip per relaunch (chips render only when ≥2 agents, so the symptom is "two Claude icons for one instance"). Same fixes any "sessions from days ago still `active`".
- **"Quit Completely" must tree-kill the pty-daemon explicitly** (`killAllPtyDaemons` in `apps/desktop/src/main/lib/pty-daemon-cleanup.ts`, called from the `forceFullCleanup` branch of `before-quit` in `main/index.ts`). The daemon is a *grandchild* (Electron → host-service → daemon), spawned `detached: true` so it survives host-service restarts (PTYs reattach on relaunch). `before-quit`'s `stopAll()` only SIGTERMs host-service; nothing reaped the detached daemon, so after "Quit Superset Completely" a `Superset.exe … pty-daemon.js` process (and its shell PTYs) lingered and had to be killed via Task Manager — despite the confirm dialog promising "all background services will be stopped". The fix reads each daemon pid from `$SUPERSET_HOME_DIR/host/{orgId}/pty-daemon-manifest.json` and `taskkill /T /F`s the tree. Only fires on full teardown (Quit Completely / dev) — a plain "Close Superset" deliberately leaves the daemon alive. Not Windows-specific in principle (POSIX detached daemon orphans the same way), just gated behind the same branch cross-platform.

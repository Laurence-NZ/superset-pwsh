...
- **Capture desktop dev logs with Turbo stream mode:** use `bun run dev:desktop --ui=stream --log-order=stream --no-color` when the TUI hides combined output. After an automated run times out, check for and stop its surviving Bun/Turbo process tree before retrying; Windows child processes can keep the dev ports bound.
- **Use Node 22 for Windows dependency installs.** Node 24 can let Electron's `extract-zip` postinstall exit successfully before `path.txt` and `dist/electron.exe` exist. If Electron Vite reports `Error: Electron uninstall`, switch to Node 22 and run `bun install --force`, or run the affected Electron package's `install.js` with Bun and verify both files exist.

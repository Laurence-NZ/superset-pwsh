...
- **`createClaudeSettingsJson` is a no-op on win32** (`apps/desktop/src/main/lib/agent-setup/agent-wrappers-claude-codex-opencode.ts`). Windows users wire their own notify hook in `~/.claude/settings.json` (`bash superset-notify.sh` → `$SUPERSET_HOME_DIR/hooks/notify.sh`), so injecting Superset's managed `cmd.exe … notify.cmd` hooks just double-POSTs per event and leaves a permanent dirty diff in dotfiles that track settings.json. `createNotifyScript` still runs, so `notify.sh` (which the user's bridge calls) is still written. Symptom the skip removes: `~/.claude/settings.json` regrows `cmd.exe … %SUPERSET_HOME_DIR%\hooks\notify.cmd` blocks on every app launch.

...
export function readProcessTable(
	...
	// Windows has no `ps`; the POSIX process table is empty here (W1). Windows
	// process cleanup runs through taskkill elsewhere.
 ...
 * as an empty table.
 */
export function readProcessTableAsync(
	platform: NodeJS.Platform = process.platform,
): Promise<ProcessInfo[] | null> {
	// Windows: no `ps`; the POSIX table is empty here (W1), matching
	// readProcessTable — not a ps failure, so resolve [] rather than null.

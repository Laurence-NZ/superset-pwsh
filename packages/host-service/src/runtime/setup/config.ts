 ...
 *   2. Fallback: `.superset/<key>.<ext>`, worktree first (when in scope), then
 *      the main repo — gitignored scripts only exist in the main repo. On
 *      Windows the portable `.ts` script wins, then native `.cmd`/`.bat`/`.ps1`,
 *      then `.sh`; elsewhere `.sh` wins, then the portable `.ts`. The command
 *      builders (setup/teardown) invoke the resolved path per its extension.
 *
...
export function resolveScript(
	...
	const extensions =
		platform === "win32" ? ["ts", "cmd", "bat", "ps1", "sh"] : ["sh", "ts"];

...
> {
	...
	// ConPTY can report pid=0 until the data pipe is ready. Keep the refresh
	// path while also replaying the daemon ring to rebuild terminal modes.
	...
	app.get(
		...
		upgradeWebSocket((c) => {
			...
			> => {
				...
				// Signal "ended" so the pane tombstones read-only. The user can
				// resume the ended agent in a fresh terminal or open a new shell.
				return { error: SESSION_ENDED_ERROR };
			};

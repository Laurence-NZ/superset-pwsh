...
{
  "setup": ["bun ./.superset/setup.ts"],
  "teardown": ["bun ./.superset/teardown.ts"],
  "run": ["bun run dev"]
}
```
...
Use portable Bun/Node scripts when the same config needs to work for every
teammate. If a project needs platform-specific scripts, commit the native entry
point for each platform and point your config at the one your team uses:

```json
{
  "setup": ["powershell -ExecutionPolicy Bypass -File ./.superset/setup.ps1"],
  "teardown": ["./.superset/teardown.cmd"]
}
```

Unix shell scripts such as `./.superset/setup.sh` still work on macOS, Linux,
and Git Bash, but they are not required for Windows-native projects.
...
```json
{
  "setup": ["bun ~/.superset/projects/<repo-path>/setup.ts"],
  "teardown": ["bun ~/.superset/projects/<repo-path>/teardown.ts"]
}
```
...
```json
{
  "setup": {
    ...
    "after": ["bun ./.superset/my-post-setup.ts"]
  },
  "teardown": {
    "after": ["bun ./.superset/my-cleanup.ts"]
  }
}
```
...
```json
{
  "setup": ["bun ./.superset/my-custom-setup.ts"]
}
```
...
```json
{
  ...
  "teardown": ["bun my-custom-teardown.ts"]
}
```
...
- Use portable Bun/Node scripts for complex cross-platform logic: `"setup": ["bun ./.superset/setup.ts"]`
- Use Windows-native scripts (`.ps1`, `.cmd`, `.bat`) when your project is Windows-specific; use `.sh` only when your team runs a Unix shell.

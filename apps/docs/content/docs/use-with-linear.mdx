...
- The [Superset CLI](/cli/getting-started) installed and signed in
  (`superset auth login`).
...
On macOS or Linux, save this as `~/.linear/open-in-superset.sh` and replace
`PROJECT` with the ID from above:
...
On Windows, save this as `%USERPROFILE%\.linear\open-in-superset.ps1`:
...
$Name = if ($env:LINEAR_ISSUE_IDENTIFIER) { $env:LINEAR_ISSUE_IDENTIFIER } else { "linear-task" }
$Branch = if ($env:LINEAR_ISSUE_BRANCH_NAME) { $env:LINEAR_ISSUE_BRANCH_NAME } else { $Name }
$Prompt = if ($env:LINEAR_PROMPT) { $env:LINEAR_PROMPT } else { "Work on Linear issue $Name." }

superset start --daemon *> $null

$Result = superset workspaces create `
  --local `
  --project $Project `
  --name $Name `
  --branch $Branch `
...
superset workspaces open $Workspace.workspace.id
```

Then save this launcher as `%USERPROFILE%\.linear\open-in-superset.cmd`:

```batch
@echo off
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%USERPROFILE%\.linear\open-in-superset.ps1"
```
...
<Callout type="info">
With the Linear integration connected, Superset does this natively: linking
...
On Windows, point Linear at the `.cmd` launcher:

```json
{
  "openIssue": {
    "path": "C:\\Users\\you\\.linear\\open-in-superset.cmd",
    "env": [
      "LINEAR_ISSUE_IDENTIFIER",
      "LINEAR_ISSUE_BRANCH_NAME",
      "LINEAR_PROMPT"
...
<Callout type="info">
In macOS file dialogs, press **⌘⇧.** to reveal hidden dot-folders, or
**⌘⇧G** to type a path directly. On Windows, paste the full path into the file
picker address bar.
</Callout>

...
## The Protocol: NDJSON Over Local Sockets

Communication between the main process and daemon uses newline-delimited JSON over a local OS transport: Unix domain sockets on POSIX and named pipes on Windows:
...
**Why local sockets?** They avoid TCP overhead, stay within the local OS security boundary, and support native backpressure through kernel buffers.
